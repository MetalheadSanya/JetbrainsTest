...
class TestListLexerSpec : StringSpec() {
    init {
        ...
        "should parse tree with TypeParam" {
            ...
            val reader = BufferedReader(StringReader(output))
            val lexer = TestListLexer()
            lexer.parseToken(reader).shouldBe(Token(Token.Type.TEST_CASE, "PrimeTableTest/0"))
        ...
        "should parse tree with GetParam" {
            ...
            lexer.parseToken(reader).shouldBe(Token(Token.Type.TEST_FUNCTION, "ReturnsFalseForNonPrimes/0"))
            lexer.parseToken(reader).shouldBe(Token(Token.Type.TEST_FUNCTION, "ReturnsFalseForNonPrimes/1"))
            lexer.parseToken(reader).shouldBe(null)
        }
